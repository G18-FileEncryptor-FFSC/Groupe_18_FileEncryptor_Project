  ...
  Future<void> _pickFile() async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: Text('Impossible de lire le fichier : ${error.message}')),
      );
    }
  }

  Future<void> _startDecryption() async {
    final inputPath = _state.selectedFilePath;
    if (inputPath == null || inputPath.isEmpty) {
      _snack('Veuillez sélectionner un fichier chiffré');
      return;
    }
    if (_passwordController.text.isEmpty) {
      _snack('Veuillez saisir le mot de passe');
      return;
    }
    ...
    setState(() {
      _state = result.copyWith(
        selectedFilePath: selectedState.selectedFilePath,
        fileName: selectedState.fileName,
        fileSizeBytes: selectedState.fileSizeBytes,
        password: '',
        clearOutputPath: false,
        clearErrorMessage: false,
      );
