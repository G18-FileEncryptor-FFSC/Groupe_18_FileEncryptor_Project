...
void main() {
  ...
  test(
    'affiche le menu de sélection si aucun argument '
    'n’est fourni en mode non interactif',
    () async {
      final io = BufferIo();
      ...
      expect(
        joined,
        contains('Que souhaitez-vous faire ?'),
      );
      expect(
        joined,
        contains('1) Chiffrer un fichier'),
      );
      expect(
        joined,
        contains('2) Déchiffrer un fichier'),
      );
      expect(
        joined,
        contains('3) Voir l’historique'),
      );
      expect(
        joined,
        contains('4) Quitter'),
  ...
  test(
    'round trip encrypt/decrypt avec un fichier temporaire',
    () async {
      final tempDir =
      ...
      expect(
        await File(restoredPath).exists(),
        isTrue,
      );

      final restoredContent =
          await File(restoredPath).readAsString();
